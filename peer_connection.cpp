...
Connection *_peer_accept(Connection &self){
	...
	Connection *peer = 0;
	if((peer = self._next->accept(*self._next))){
...
int _peer_send(Connection &self, const char *data, size_t size){
	...
	Packet pack;
	size = min(size, ARRSIZE(pack.data));
...
static void _con_send_error(Connection &self, ConnectionError err, const string &description){
	...
	pack.cmd.size = min(ARRSIZE(pack.data), (description.length()+sizeof(ConnectionError)));
...
static void _con_handle_packet(Connection &self, const Packet &packet){
	...
	if(packet.cmd.code == CMD_DATA && self._prev){
		self._prev->send(*self._prev, &packet.data[0], packet.cmd.size); 
		LOG("[bridge] forwarded "<<packet.cmd.size<<" bytes to external socket!");
	}
	// if we have a listener socket then we write the data to it's input buffer as well
		...
		self._prev->on_data_received(*self._prev, &packet.data[0], packet.cmd.size);
		LOG("[bridge] forwarding "<<packet.cmd.size<<" bytes to internal socket "<<
	...
	/// we have received a request to extend our connection to another host
		...
		if(proto == "udt"){
			Connection *c = self._prev = NET_allocConnection(*self.net);
			CON_initUDT(*c);
		...
		if(packet.cmd.size > sizeof(ConnectionError)){
			ConnectionError err;
			
			LOG("[error] "+string(packet.data+sizeof(ConnectionError)));
	...
	if(self.state == CON_STATE_ESTABLISHED){
		...
		if(self._prev){
			...
			if(ss>0){
				...
				pack.cmd.code = CMD_DATA;
				pack.cmd.size = ss;
				memcpy(pack.data, tmp, min(ARRSIZE(pack.data), (unsigned long)ss));
				// forward the data to the connection higher up in queue
				self._next->send(*self._next, pack.c_ptr(), pack.size());
			}
		} 
