...
int con_send(Connection *self, const char *buf, size_t size){
	...
	// if we are bridged directly to another socket
	if(self->proto != REL_PROTO_INTERNAL_CLIENT && self->proto != REL_PROTO_INTERNAL_SERVER){
		rc = BIO_write(self->write_buf, buf, size);
	}
	else if((rc = SSL_write(self->con, buf, size))<=0){
		LOG("error sending ssl to "<<self->host<<":"<<self->port<<": "<<errorstring(SSL_get_error(self->con, rc)));
		return -1;
	}
...
int con_recv(Connection *self, char *rcv_buf, size_t size){
	...
	if(self->proto != REL_PROTO_INTERNAL_SERVER && self->proto != REL_PROTO_INTERNAL_CLIENT){
		rc = BIO_read(self->read_buf, rcv_buf, size);
	}
	else if((rc = SSL_read(self->con, rcv_buf, size)) <= 0){
		// TODO: handle disconnect here
		// read could also fail simply because not enough data was received
		//LOG("[error] ssl read failed!");
		return -1;
	}
	
	if(rc>0){
		LOG("[connection] recv: "<<self->host<<":"<<self->port<<" length: "<<rc);
...
void con_process(Connection *self){
	...
	// send and receive needs to be done every frame to ensure that handshakes 
	// and other multiframe operations complete correctly
	if(!self->_bridge_next){
		...
		/// if we are bridged then we only send
...
void CON_bridge(Connection *self, Connection *other){
	...
	if(self->proto == REL_PROTO_INTERNAL_CLIENT){
		...
		LOG("bridging server connection!");
